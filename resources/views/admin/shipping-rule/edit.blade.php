                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4>Chỉnh sửa thông tin vận chuyển</h4>
                                </div>
                                <div class="card-body">
                                    <form action="{{ route('admin.shipping-rule.update', $shippingRule->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('PUT')
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="type" id="" class="form-control shipping-type">
                                                <option {{ $shippingRule->type == 'flat_cost' ? 'selected' : '' }}
                                                    value="flat_cost">Giá cố định</option>
                                                <option {{ $shippingRule->type == 'min_cost' ? 'selected' : '' }}
                                                    value="min_cost">Theo giá trị tối thiểu của đơn hàng</option>
                                            </select>
                                            @if ($errors->has('type'))
                                        ...
                                        <div
                                            class="form-group min_cost {{ $shippingRule->type == 'min_cost' ? '' : 'd-none' }}">
                                            <label>Giá trị đơn hàng tối thiểu (VNĐ)</label>
                                            <input type="number" class="form-control" name="min_cost"
                                                placeholder="Ví dụ: 0, 100.000,..."
                                                value="{{ old('min_cost', $shippingRule->min_cost) }}">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="number" class="form-control" name="cost"
                                                placeholder="Ví dụ: 0, 50.000,..."
                                                value="{{ old('cost', $shippingRule->cost) }}">
                                            @if ($errors->has('cost'))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="status" id="" class="form-control">
                                                <option {{ $shippingRule->type == 1 ? 'selected' : '' }} value="1">
                                                    Hiển thị</option>
                                                <option {{ $shippingRule->type == 0 ? 'selected' : '' }} value="0">
