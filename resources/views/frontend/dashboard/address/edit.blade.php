    ...
    <div class="row">
        <div class="col-xl-9 col-xxl-10 col-lg-9 ms-auto">
            <div class="dashboard_content mt-2 mt-md-0">
                ...
                <h3><i class="fal fa-map"></i>Chỉnh sửa địa chỉ</h3>
                            ...
                            <div class="col-xl-6 col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input class="form-control" type="email" name="email"
                                        placeholder="example@gmail.com" value="{{ old('name', $address->email) }}">
                                    @if ($errors->has('email'))
                                        <p class="text-danger d-flex justify-content-end mt-1">
                                            {{ $errors->first('email') }}
                                        </p>
                ...
                if (data_tinh.error == 0) {
                    $.each(data_tinh.data, function(key_tinh, val_tinh) {
                        $("#tinh").append('<option value="' + val_tinh.id + '">' + val_tinh
                            .full_name + '</option>');
                        if (val_tinh.full_name == "{{ $address->province_city }}") {
                            $("#tinh").val(val_tinh.id);
                            // Gọi hàm xử lý cho quan và phuong sau khi thiết lập giá trị mặc định cho tinh
                            handleDistrictAndWardChange(val_tinh.id);
                        }
                    });

                    // Xử lý sự kiện thay đổi giá trị của select box tinh
                    $("#tinh").change(function() {
                        var idtinh = $(this).val();
                        handleDistrictAndWardChange(idtinh);
                    });
                }
            });
                ...
                $.getJSON('/fetch-tinhthanh/2/' + idtinh, function(data_quan) {
                    if (data_quan.error == 0) {
                        ...
                        $.each(data_quan.data, function(key_quan, val_quan) {
                            $("#quan").append('<option value="' + val_quan.id + '">' + val_quan
                                .full_name + '</option>');
                            if (val_quan.full_name == "{{ $address->district }}") {
                                $("#quan").val(val_quan.id);
                                // Gọi hàm xử lý cho phuong sau khi thiết lập giá trị mặc định cho quan
                                handleWardChange(val_quan.id);
                            }
                        });

                        // Xử lý sự kiện thay đổi giá trị của select box quan
                        $("#quan").change(function() {
                            var idquan = $(this).val();
                            handleWardChange(idquan);
                        });
                    }
                });
            }
                ...
                $.getJSON('/fetch-tinhthanh/3/' + idquan, function(data_phuong) {
                    if (data_phuong.error == 0) {
                        ...
                        $.each(data_phuong.data, function(key_phuong, val_phuong) {
                            ...
                            if (val_phuong.full_name == "{{ $address->commune_ward }}") {
                                $("#phuong").val(val_phuong.id);
                            }
                        });
                    }
                });
