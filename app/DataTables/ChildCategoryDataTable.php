...
class ChildCategoryDataTable extends DataTable
{
  ...
  public function dataTable(QueryBuilder $query): EloquentDataTable
  {
    return (new EloquentDataTable($query))
      ->addColumn('action', function ($query) {
        $editBtn = "<a href='" . route('admin.child-category.edit', $query->id) . "' class='btn btn-primary mr-2'>
        <i class='fas fa-pen'></i>
      </a>";
        $deleteBtn = "<a href='" . route('admin.child-category.destroy', $query->id) . "' class='btn btn-danger delete-item'>
  ...
  public function getColumns(): array
  {
    return [
      ...
      Column::make('slug'),
      Column::make('category_id')->width(200)->title("Thuộc danh mục cấp 1"),
      Column::make('sub_category_id')->width(200)->title("Thuộc danh mục cấp 2"),
